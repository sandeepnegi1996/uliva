...
export default function CartPage() {
  ...
  return (
    <section className="mx-auto w-full max-w-[1360px] px-4 py-10 md:px-6 lg:px-8" aria-label="Shopping cart">
      <h1 className="text-3xl font-black uppercase tracking-[-0.05em] text-[#153d30] dark:text-[#c4e0a8]">
        Cart <span aria-live="polite" aria-atomic="true">{itemCount > 0 ? `(${itemCount})` : ""}</span>
      </h1>
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_340px]">
          <ul className="space-y-4" aria-label="Cart items list">
            {items.map((item) => (
              <li key={item.id} className="flex gap-4 rounded-2xl border border-[#e8e2d6] bg-white p-4 dark:border-stone-700 dark:bg-[#222a24]">
                ...
                <div className="flex flex-1 items-start justify-between gap-3">
                  ...
                  <p className="text-sm font-black text-[#153d30] dark:text-[#c4e0a8]" aria-label={`Total price: ${formatPrice(item.price * item.quantity)}`}>{formatPrice(item.price * item.quantity)}</p>
                </div>
              </li>
            ))}
          </ul>

          <aside className="h-fit rounded-2xl border border-[#e8e2d6] bg-white p-5 dark:border-stone-700 dark:bg-[#222a24]" aria-label="Cart summary">
            <p className="text-[0.62rem] font-bold uppercase tracking-[0.18em] text-[#5d6c67] dark:text-[#8a9a94]">Subtotal</p>
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-lg font-black text-[#153d30] dark:text-[#c4e0a8]" aria-live="polite" aria-atomic="true">{formatPrice(subtotal)}</span>
